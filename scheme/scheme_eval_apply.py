...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        return scheme_apply(procedure, expr.rest.map(lambda e: scheme_eval(e, env)), env)
        # END PROBLEM 3
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    def check(exprs, env):
        first, rest = exprs.first, exprs.rest
        if exprs.rest == nil:
            return scheme_eval(first, env)
        else:
            scheme_eval(first, env)
            return check(rest, env)
    return  check(expressions, env)
    # END PROBLEM 6
