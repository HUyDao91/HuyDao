...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambdaFunc = do_lambda_form(Pair(expressions.first.rest, expressions.rest), env)
        env.define(expressions.first.first, lambdaFunc)
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    if expressions == nil or expressions.first == nil:
        return True
    evalRes = scheme_eval(expressions.first, env)
    if is_scheme_false(evalRes):
        return evalRes
    else:
        return do_and_form(expressions.rest, env) if expressions.rest != nil else evalRes
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions == nil or expressions.first == nil:
        return False
    evalRes = scheme_eval(expressions.first, env)
    if is_scheme_true(evalRes):
        return evalRes
    else:
        return do_or_form(expressions.rest, env) if expressions.rest != nil else evalRes
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    def helper(binds):
        if binds == nil:
            ...
            this = binds.first
            validate_form(this, 2, 2)
            name = Pair(this.first, nil)
            nextHead = this.rest.first
            val = Pair(scheme_eval(nextHead, env), nil)
            name.rest, val.rest = helper(binds.rest)
            return (name, val)
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    mu = MuProcedure(expressions.first, expressions.rest)
    return mu
    # END PROBLEM 11
