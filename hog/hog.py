...
def roll_dice(num_rolls, dice=six_sided):
    ...
    sum = 0
    while num_rolls > 0:
        face = dice()       #using another for dice(), will help stop calling face() and able to check the current value of each face()
        if face == 1:
            sum = 1
        elif sum != 1:
            sum += face
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # orginal_function take (1 , dice) >>> pass the agrument(1, dice) to roll_dice which is original fucntion(*args = # of agruments)
    # >>> calculate the average of each call >>> take the sum of all average / total_samples
    
    #create func that return original_function with its according arguments.
    # def each_func(*args):  
    #     result = original_function(*args)
    #     return result
    
    return lambda *args: sum(original_function(*args) for num in range(0,total_samples))/ total_samples
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    best_roll_score, best_roll_dice = -1, 0
    ave = make_averaged(roll_dice,total_samples)
    for num in range(1,11):
        score = ave(num, dice)
        if(score>best_roll_score):
            best_roll_score, best_roll_dice = score, num
    return best_roll_dice
    # END PROBLEM 9
