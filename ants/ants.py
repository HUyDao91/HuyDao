...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.is_container is True and self.can_contain(place.ant):
                x = place.ant
                place.ant = self
                self.store_ant(x)
    ...
    def double(self):
        ...
        if self.double_damage == False:
            ...
            #return self.action(self.damage*2)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        next_location = self.place
        distance = 0
        while not next_location.is_hive:
            if self.lower_bound <= distance <= self.upper_bound and next_location.bees :
                return random_bee(next_location.bees)
            next_location = next_location.entrance
            distance +=1 
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    def __init__(self, upper_bound = 3):
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    def __init__(self, lower_bound = 5):
...
class WallAnt(Ant):
    ...
    def __init__(self,health=4):
        #self.health =health
        super().__init__(health)# why is this health and not self.health?
# END Problem 6
...
# if queen health hits 0 override Insect.reduce_health in QueenAnt and call ants_lose()
# only one queen - Ant.construct() class method to either construct an Ant if possible, or return None if not
# doubles damage of ants behind her for every two turns
# queen ant cannot be removed so override Ant.remove_from in QueenAnt

#Questions for 12
#how do i know when the ant is the Fireant so i dont increase its damagee
# how do i mark the ants that have been double damaged- do i do that in the actions class?
#is my damage() ok
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        if gamestate.queen == False:
            ...
            #we do not need cls because super() is the first aarg for construct
        else:
            return None
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        while curr is not None:
            ...
            curr = curr.exit
                ...
                #how do idifferentiate container ant and ant being contained
            
                

 
        #while not none
        #impleement requirements
        # -check if place has an ant if curr.ant the double ant damage
        #            -if containerant and its holding an ant double both damage
        # if we have multiple places behind curr aka curr =curr.exit
        #  
            #if so dble the ant damage
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem EC
        target.slow = 5
        def t_action(gamestate):
            if gamestate.time%2 == 0:
                Bee.action(target,gamestate)
        target.action =t_action()
        # END Problem EC
